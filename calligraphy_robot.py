...
# pyserial — for sending G-code to STM32
try:
    import serial
...
FONT_INPUT = ("Georgia", 14)

MODEL_PATH   = "calligraphy_model.pkl"
DEFAULT_PORT = "/dev/ttyAMA0"   # hardware UART on Raspberry Pi 5
DEFAULT_BAUD = 115200
...
class NumpyLSTM:
    def __init__(self, vocab=VOCAB, embed_dim=48,
        ...
        H, I     = hidden, embed_dim

        self.Wx1 = np.random.randn(I, 4*H) * 0.05
        self.Wh1 = np.random.randn(H, 4*H) * 0.05
        self.b1  = np.zeros(4*H);  self.b1[H:2*H] = 1.0

        self.Wx2 = np.random.randn(H, 4*H) * 0.05
        self.Wh2 = np.random.randn(H, 4*H) * 0.05
        self.b2  = np.zeros(4*H);  self.b2[H:2*H] = 1.0

        self.Wo  = np.random.randn(H, out_dim) * 0.05
    ...
    def _pnames(self):
        return ["emb","Wx1","Wh1","b1","Wx2","Wh2","b2","Wo","bo"]
    ...
    def _adam_update(self, grads, b1=0.9, b2=0.999, eps=1e-8):
        ...
        for k, g in grads.items():
            ...
            setattr(self, k, getattr(self, k) - lr_t*self._m[k]/(np.sqrt(self._v[k])+eps))
    ...
    def backward(self, char_seq, targets, outputs, cache):
        ...
        grads = {k: np.zeros_like(getattr(self, k)) for k in self._pnames()}
        dh2 = np.zeros(H); dc2 = np.zeros(H)
        dh1 = np.zeros(H); dc1 = np.zeros(H)
        total_loss = 0.0

        for t in reversed(range(T)):
            pred = outputs[t]; tgt = targets[t]
            d_dx  = pred[0]-tgt[0]; d_dy = pred[1]-tgt[1]; d_pen = pred[2]-tgt[2]
    ...
    def save(self, path=MODEL_PATH):
        data = {k: getattr(self, k) for k in self._pnames()}
        data.update({"V":self.V,"E":self.E,"H":self.H,"out":self.out,"lr":self.lr})
    ...
    try:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for start in range(0, n, batch):
                ...
                b_loss = 0.0
                for i in b_idx:
                    outs, cache = model.forward(X[i])
                    b_loss += model.backward(X[i], Y[i], outs, cache)
            ...
            if epoch % 10 == 0 or epoch == 1:
                log_cb(f"Epoch {epoch:3d}/{epochs}  loss={avg:.5f}  best={best_loss:.5f}")
...
def _letter_segments(ch: str):
    lo = ch.lower()
    if lo == 'a':  return [[(0,0),(5,20),(10,0)], [(2,8),(8,8)]]
    elif lo == 'b': return [[(0,0),(0,20),(7,20),(9,17),(9,13),(7,10),(0,10)],
                             [(0,10),(7,10),(9,7),(9,3),(7,0),(0,0)]]
    elif lo == 'c': return [[(9,16),(6,20),(3,20),(1,17),(0,13),(0,7),(1,3),(3,0),(6,0),(9,4)]]
    elif lo == 'd': return [[(0,0),(0,20),(5,20),(9,16),(9,4),(5,0),(0,0)]]
    elif lo == 'e': return [[(9,0),(0,0),(0,20),(9,20)], [(0,10),(7,10)]]
    elif lo == 'f': return [[(0,0),(0,20),(9,20)], [(0,12),(6,12)]]
    elif lo == 'g': return [[(9,16),(6,20),(3,20),(1,17),(0,13),(0,7),(1,3),(3,0),(6,0),(9,3),(9,10),(5,10)]]
    elif lo == 'h': return [[(0,0),(0,20)], [(0,10),(9,10)], [(9,20),(9,0)]]
    elif lo == 'i': return [[(2,20),(8,20)], [(5,20),(5,0)], [(2,0),(8,0)]]
    elif lo == 'j': return [[(2,20),(8,20)], [(5,20),(5,3),(3,0),(1,0),(0,2)]]
    elif lo == 'k': return [[(0,0),(0,20)], [(9,20),(0,10),(9,0)]]
    elif lo == 'l': return [[(0,20),(0,0),(9,0)]]
    elif lo == 'm': return [[(0,0),(0,20),(5,10),(10,20),(10,0)]]
    elif lo == 'n': return [[(0,0),(0,20),(9,0),(9,20)]]
    elif lo == 'o': return [[(1,0),(0,4),(0,16),(1,20),(4,20),(6,20),(9,16),(9,4),(6,0),(4,0),(1,0)]]
    elif lo == 'p': return [[(0,0),(0,20),(7,20),(9,17),(9,13),(7,10),(0,10)]]
    elif lo == 'q': return [[(1,4),(0,8),(0,16),(1,20),(4,20),(6,20),(9,16),(9,4),(6,0),(4,0),(1,4)],[(7,3),(10,0)]]
    elif lo == 'r': return [[(0,0),(0,20),(7,20),(9,17),(9,13),(7,10),(0,10)], [(4,10),(9,0)]]
    elif lo == 's': return [[(9,17),(7,20),(3,20),(1,18),(1,14),(3,12),(7,8),(9,6),(9,3),(7,0),(3,0),(1,3)]]
    elif lo == 't': return [[(5,20),(5,0)], [(1,20),(9,20)]]
    elif lo == 'u': return [[(0,20),(0,4),(2,0),(5,0),(8,0),(10,4),(10,20)]]
    elif lo == 'v': return [[(0,20),(5,0),(10,20)]]
    elif lo == 'w': return [[(0,20),(2,0),(5,10),(8,0),(10,20)]]
    elif lo == 'x': return [[(0,20),(10,0)], [(10,20),(0,0)]]
    elif lo == 'y': return [[(0,20),(5,10),(10,20)], [(5,10),(5,0)]]
    elif lo == 'z': return [[(0,20),(10,20),(0,0),(10,0)]]
    elif ch == '0': return [[(1,0),(0,4),(0,16),(1,20),(4,20),(6,20),(9,16),(9,4),(6,0),(4,0),(1,0)],[(2,4),(8,16)]]
    elif ch == '1': return [[(2,16),(5,20),(5,0)], [(2,0),(8,0)]]
    elif ch == '2': return [[(1,16),(2,20),(7,20),(9,16),(9,12),(1,4),(1,0),(9,0)]]
    elif ch == '3': return [[(1,20),(8,20),(9,16),(9,13),(7,11),(4,10),(7,9),(9,7),(9,3),(7,0),(1,0)]]
    elif ch == '4': return [[(7,0),(7,20),(0,8),(10,8)]]
    elif ch == '5': return [[(9,20),(1,20),(1,12),(7,12),(9,9),(9,3),(7,0),(1,0)]]
    elif ch == '6': return [[(8,18),(5,20),(3,20),(1,16),(0,10),(0,4),(2,0),(5,0),(8,0),(9,4),(9,8),(7,11),(4,11),(1,10)]]
    elif ch == '7': return [[(0,20),(10,20),(4,0)]]
    elif ch == '8': return [[(5,10),(8,13),(8,17),(5,20),(2,17),(2,13),(5,10),(8,7),(8,3),(5,0),(2,3),(2,7),(5,10)]]
    elif ch == '9': return [[(1,2),(4,0),(7,0),(9,4),(9,10),(7,13),(4,13),(1,10),(1,6),(3,13),(5,20),(7,18)]]
    elif ch == '.': return [[(4,0),(5,0),(5,1),(4,1),(4,0)]]
    elif ch == ',': return [[(4,1),(5,1),(5,2),(4,0),(3,-1)]]
    elif ch == '!': return [[(5,20),(5,6)], [(5,2),(5,0)]]
    elif ch == '?': return [[(1,16),(2,20),(7,20),(9,16),(9,12),(5,8),(5,4)], [(5,1),(5,0)]]
    elif ch == ' ': return []
...
def build_gcode(text: str, model: NumpyLSTM,
    ...
    lines = [
        ...
        "; Structured templates + RNN style variation",
        "; Generated on Raspberry Pi 5",
        "",
        "G21         ; millimeters",
    ...
    for i, ch in enumerate(text):
        ...
        for seg in segments:
            ...
            for pt in seg[1:]:
                ...
                ax = max(0.0, min(cursor_x + jx, 250.0))
                ay = max(0.0, min(Y_BASELINE + jy, 250.0))
    ...
    lines += [
        f"M3 S{pen_up_angle}      ; pen up final",
        "G28         ; return home",
        "; === End ===",
    ]
    return lines
...
def serial_send(gcode_lines: list, port: str, baud: int,
    ...
    try:
        log_cb(f"Opening {port} at {baud} baud...")
        with serial.Serial(port, baud, timeout=2) as ser:
            # Wait for STM32 to boot / flush startup noise
            time.sleep(2)
            ser.flushInput()
            ...
            for i, cmd in enumerate(commands):
                ...
                # Small delay between commands to not overflow STM32 buffer
                time.sleep(0.01)

        log_cb("All commands sent successfully.")
        done_cb(success=True)
...
class CalligraphyApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Calligraphy Robot — Raspberry Pi 5")
    ...
    def _check_model_on_start(self):
        if os.path.exists(MODEL_PATH):
            try:
                ...
                self._set_status(f"Could not load model: {e}", WARNING)
                self._gen_btn.config(state="disabled")
        else:
            self._set_status("No model found — press  TRAIN MODEL  first.", WARNING)
            self._gen_btn.config(state="disabled")
    ...
    def _refresh_ports(self):
        ...
        # Always include the Pi5 hardware UART at the top
    ...
    def _build_header(self):
        ...
        tk.Label(hdr, text="CALLIGRAPHY ROBOT",
                 font=FONT_TITLE, bg=BG, fg=ACCENT).grid(row=0, column=0, sticky="w")
        tk.Label(hdr, text="NumPy LSTM  ·  Raspberry Pi 5  ·  STM32L476",
                 font=FONT_SUB, bg=BG, fg=TEXT_DIM).grid(
                 row=0, column=1, sticky="w", padx=14)

        # Serial port selector on the right
        serial_frame = tk.Frame(hdr, bg=BG)
        serial_frame.grid(row=0, column=2, sticky="e")

        tk.Label(serial_frame, text="PORT", font=FONT_MONO,
                 bg=BG, fg=TEXT_DIM).pack(side="left", padx=(0,4))

        self._port_var = tk.StringVar(value=DEFAULT_PORT)
        self._port_cb  = ttk.Combobox(serial_frame, textvariable=self._port_var,
                                       width=16, state="readonly")
        self._port_cb.pack(side="left")

        tk.Button(serial_frame, text="⟳", font=FONT_MONO,
                  bg=SURFACE2, fg=ACCENT, relief="flat", bd=0,
                  padx=6, cursor="hand2",
                  command=self._refresh_ports).pack(side="left", padx=4)

        tk.Label(serial_frame, text="BAUD", font=FONT_MONO,
                 bg=BG, fg=TEXT_DIM).pack(side="left", padx=(8,4))
        self._baud_var = tk.StringVar(value=str(DEFAULT_BAUD))
        ttk.Combobox(serial_frame, textvariable=self._baud_var,
                     values=["9600","57600","115200","230400"],
                     width=8, state="readonly").pack(side="left")

        tk.Frame(self, bg=ACCENT, height=1).grid(row=0, column=0, sticky="sew")

    def _build_input_section(self):
        outer = tk.Frame(self, bg=BG)
        outer.grid(row=1, column=0, sticky="ew", padx=30, pady=(14,0))
        ...
        self._text_entry.bind("<KeyRelease>",
            lambda e: self._char_var.set(f"{len(self._text_entry.get())} chars"))
        ...
        self._send_btn = self._btn(
            btn_row, "  SEND TO STM32", BLUE, BG,
            self._on_send, state="disabled")
        ...
        self._copy_btn = self._btn(
            btn_row, "  COPY", SURFACE2, TEXT_DIM,
    ...
    def _build_progress_row(self):
        ...
        style.configure("Green.Horizontal.TProgressbar",
                         ...
                         bordercolor=SURFACE2, lightcolor=SUCCESS, darkcolor="#3a7a3a")
        style.configure("Blue.Horizontal.TProgressbar",
                         troughcolor=SURFACE2, background=BLUE,
                         bordercolor=SURFACE2, lightcolor=BLUE, darkcolor="#2e5080")

        # Training progress
        tk.Label(outer, text="TRAIN", font=FONT_MONO,
                 bg=BG, fg=TEXT_DIM).grid(row=0, column=0, padx=(0,6))
        self._train_prog = ttk.Progressbar(outer, length=220,
                                            style="Gold.Horizontal.TProgressbar")
        self._train_prog.grid(row=0, column=1, padx=(0,4))
        self._train_pct = tk.StringVar(value="")
        tk.Label(outer, textvariable=self._train_pct,
                 font=FONT_MONO, bg=BG, fg=ACCENT, width=5).grid(row=0, column=2)

        # Inference progress
        tk.Label(outer, text="INFER", font=FONT_MONO,
                 bg=BG, fg=TEXT_DIM).grid(row=0, column=3, padx=(16,6))
        self._infer_prog = ttk.Progressbar(outer, length=160,
                                            style="Green.Horizontal.TProgressbar")
        self._infer_prog.grid(row=0, column=4, padx=(0,4))
        ...
        self._send_prog.grid(row=0, column=7, padx=(0,4))
        self._send_pct = tk.StringVar(value="")
        tk.Label(outer, textvariable=self._send_pct,
                 font=FONT_MONO, bg=BG, fg=BLUE, width=5).grid(row=0, column=8)
    ...
    def _build_panels(self):
        ...
        # Left: training log + serial log combined
        left = tk.Frame(pane, bg=SURFACE)
        pane.add(left, minsize=280)
        left.rowconfigure(1, weight=1)
        left.columnconfigure(0, weight=1)
        self._panel_hdr(left, "LOG  (Training · Serial)", 0)
        ...
        self._log_box.tag_config("serial", foreground="#5b9bd5")
    ...
    def _lock(self):
        ...
        for b in [self._train_btn, self._gen_btn,
                  self._send_btn, self._clear_btn, self._copy_btn]:
            b.config(state="disabled")

    def _unlock(self, model_ready=True, gcode_ready=False):
        self._running = False
        self._train_btn.config(state="normal")
        self._gen_btn.config(state="normal" if model_ready else "disabled")
        self._send_btn.config(state="normal" if gcode_ready else "disabled")
    ...
    def _on_train_done(self, success, error=""):
        if success:
            ...
            self._set_status("Training complete — enter text and press GENERATE.", SUCCESS)
            self._train_btn.config(text="  RE-TRAIN MODEL")
            self._unlock(model_ready=True, gcode_ready=bool(self._gcode))
        else:
    ...
    def _on_send(self):
        ...
        if not port or "not installed" in port:
            messagebox.showerror("No Port", "Select a valid serial port.")
            return

        self._lock()
        self._send_prog["value"] = 0
        self._send_pct.set("0%")
        self._append_log("─" * 40)
        self._append_log(f"Connecting to {port} @ {baud} baud...", "serial")
        self._set_status(f"Sending G-code to STM32 on {port}...", BLUE)

        def log_cb(msg):
            self.after(0, lambda m=msg: self._append_log(m, "serial"))
        def progress_cb(pct):
            self.after(0, lambda p=pct: (
                self._send_prog.__setitem__("value", p),
                self._send_pct.set(f"{p}%")))
        def done_cb(success=True, error=""):
            self.after(0, lambda: self._on_send_done(success, error))

        threading.Thread(
            target=serial_send,
            args=(self._gcode, port, baud, log_cb, progress_cb, done_cb),
            daemon=True).start()

    def _on_send_done(self, success, error=""):
        if success:
            self._send_prog["value"] = 100
            self._send_pct.set("100%")
            self._set_status("STM32 execution complete — calligraphy done!", SUCCESS)
            self._append_log("── TRANSMISSION COMPLETE ──", "serial")
        else:
    ...
    def _on_clear(self):
        ...
        self._gcode = []
        self._set_gcode([])
        for prog in [self._train_prog, self._infer_prog, self._send_prog]:
            prog["value"] = 0
        for var in [self._train_pct, self._infer_pct, self._send_pct]:
            var.set("")
        self._unlock(model_ready=self._model is not None, gcode_ready=False)
